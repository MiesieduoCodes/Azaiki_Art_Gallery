...
import serviceAccount from "../path-to-your-service-account.json"; // Replace with your service account file
...
// Function to upload artists
async function uploadArtists() {
  for (const artist of artists) {
    await db.collection("artists").doc(artist.id).set(artist);
    console.log(`Uploaded artist: ${artist.name}`);
  }
}

// Function to upload artworks
async function uploadArtworks() {
  for (const artwork of galleryItems) {
    await db.collection("artworks").doc(artwork.id).set(artwork);
    console.log(`Uploaded artwork: ${artwork.title}`);
  }
}
